...
# Installation
```npm install simplest-discord-webhook``` or ```yarn add simplest-discord-webhook```
...
## Custom embeds
```js
const { Webhook, MessageBuilder } = require('simplest-discord-webhook');
const hook = new Webhook("YOUR WEBHOOK URL");
...
# Notes
simplest-discord-webhook is a promise based library, which means you can use `.catch`, `.then`, and `await`, although if successful will not return any values. For example:
